...
export const PromptInputActionAddAttachments = ({
  ...
  const handleSelect = useCallback(
    // Base UI event shape differs from DOM Event; keep broad for Elements compat.
  ...
  const inner = (
    <>
      ...
      <form
        ...
        {...props}
      >
        <InputGroup className="overflow-hidden bg-background dark:bg-input/30">
          {children}
        </InputGroup>
      </form>
    </>
...
export const PromptInputTextarea = ({
  ...
  return (
    <InputGroupTextarea
      className={cn(
        "field-sizing-content max-h-48 min-h-12 px-3 py-2.5 text-sm md:text-sm",
        className,
      )}
      name="message"
...
export const PromptInputFooter = ({
  ...
  <InputGroupAddon
    align="block-end"
    className={cn("justify-between gap-1 pb-1.5 pt-0", className)}
    {...props}
  />
);
...
export const PromptInputSubmit = ({
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      // Base UI Button onClick expects BaseUIEvent; cast for Elements compat.
      (onClick as ((ev: typeof e) => void) | undefined)?.(e);
    },
...
export type PromptInputHoverCardProps = ComponentProps<typeof HoverCard> & {
  /** Accepted for Radix API parity; Base UI HoverCard ignores these. */
  openDelay?: number;
  closeDelay?: number;
};
