import type { MessageTree } from "./en";

export const zh: MessageTree = {
  app: {
    brand: "okf-wiki",
    operator: "操作台",
    sidebarAria: "主导航",
    sidebarFoot:
      "本地 Web 操作界面。模型密钥仅保存在本机，不会写入 workspace.json。",
    expandSidebar: "展开侧栏",
    collapseSidebar: "收起侧栏",
    collapse: "收起",
  },
  nav: {
    workspaces: "工作区",
    settings: "设置",
  },
  subnav: {
    aria: "工作区分区",
    overview: "概览",
    sources: "源码",
    session: "会话",
    runs: "任务",
    wiki: "Wiki",
    settings: "设置",
  },
  locale: {
    label: "界面语言",
    en: "English",
    zh: "中文",
    switchTo: "切换语言",
  },
  common: {
    loading: "加载中…",
    save: "保存更改",
    saving: "保存中…",
    saved: "已保存",
    cancel: "取消",
    create: "创建",
    delete: "删除",
    deleting: "删除中…",
    removing: "移除中…",
    on: "开",
    off: "关",
    optional: "可选",
    actions: "操作",
    error: "错误",
    dismiss: "关闭",
    confirm: "确认",
    name: "名称",
    path: "路径",
    id: "ID",
  },
  workspaces: {
    title: "工作区",
    descriptionBefore:
      "工作区是本地项目：Git 源码、所选模型与 Wiki 输出路径。模型与凭证在",
    descriptionAfter: "中管理。",
    settingsLink: "设置",
    create: "创建",
    cancel: "取消",
    createTitle: "创建工作区",
    nameLabel: "名称",
    namePlaceholder: "我的项目",
    rootLabel: "根路径（绝对路径）",
    rootPlaceholder: "D:/src/app",
    rootHint: "本机绝对路径。服务端会按需创建目录与 .okf-wiki/。",
    createSubmit: "创建工作区",
    creating: "创建中…",
    loading: "加载工作区…",
    emptyTitle: "还没有工作区",
    emptyDescription: "使用绝对根路径创建工作区，再添加本地 Git 检出作为源码。",
    checklistModels: "配置模型：",
    checklistSources: "添加源码：关联本地 Git 检出，或克隆到工作区",
    checklistRun: "生成 Wiki 并人工确认发布",
    colName: "名称",
    colRoot: "根路径",
    colSources: "源码数",
    colLastOpened: "最近打开",
    colActions: "操作",
    delete: "删除",
    deleteConfirmTitle: "删除工作区？",
    deleteConfirmBody:
      "将「{name}」从最近列表移除。除非同时删除元数据，磁盘上的项目文件会保留。",
    deleteMetaLabel: "同时删除根目录下的 .okf-wiki/（运行记录、会话、暂存）",
    deleteSubmit: "删除工作区",
    deleting: "删除中…",
  },
  detail: {
    breadcrumb: "工作区",
    titleFallback: "工作区",
    description: "本地项目配置概览。",
    loading: "加载工作区…",
    name: "名称",
    id: "ID",
    rootPath: "根路径",
    model: "模型",
    profile: "配置",
    publicationPath: "发布路径",
    orchestration: "监督者树",
    planConfirm: "计划确认",
    wikiLanguage: "Wiki 语言",
    sources: "源码",
    manageSources: "管理源码",
    created: "创建时间",
    lastOpened: "最近打开",
    editSettings: "编辑设置",
    sourcesBtn: "源码",
    langEn: "英文",
    langZh: "中文",
    nextStepsTitle: "下一步",
    ctaAddSource: "添加源码",
    ctaOpenSession: "打开智能体工作区",
    ctaOpenWiki: "浏览 Wiki",
    statusSourcesTitle: "源码",
    statusSourcesEmpty: "还没有源码 — 关联 Git 检出后即可生成 Wiki。",
    statusModelTitle: "模型",
    statusWikiTitle: "Wiki 语言",
    statusPublicationTitle: "发布路径",
  },
  settings: {
    breadcrumbWorkspaces: "工作区",
    breadcrumbSettings: "设置",
    title: "工作区设置",
    descriptionPrefix: "此工作区的项目选项。模型与凭证仅在",
    descriptionLink: "设置",
    descriptionSuffix: "中配置。",
    loading: "加载设置…",
    tabGeneral: "常规",
    tabSkill: "Skill",
    tabDanger: "危险",
    name: "名称",
    publicationPath: "发布路径（绝对路径）",
    planConfirm: "计划确认",
    planConfirmHint:
      "交互运行在 produce 前暂停，等待操作者批准 Wiki Spec（域 + 页面）。",
    wikiLanguage: "Wiki 语言",
    wikiLanguageHint: "生成 Wiki 页面标题与正文使用的语言（与操作界面语言无关）。",
    contextTargetTokens: "上下文目标（tokens）",
    contextTargetTokensHint:
      "Wiki 生成时的上下文压缩预算（Observational Memory + 硬 token 上限）。留空则在模型配置了最大上下文时使用其 85%。",
    contextTargetTokensPlaceholder: "例如 100000",
    contextTargetDerived: "当前由模型最大上下文推导：{n} tokens（85%）。",
    langEn: "English（英文）",
    langZh: "中文（简体）",
    save: "保存更改",
    saving: "保存中…",
    saved: "已保存",
    orphanModel: "先前的模型 ID 已不在设置中。请在上方选择已配置的模型。",
    skillTitle: "Producer Skill",
    skillDescription:
      "Wiki 生成的方法与模板包。默认遵循 Agent Skills 路径：项目 `.agents/skills`，再用户 `~/.agents/skills`（见全局设置），最后产品包。可创建工作区 fork 自定义模板与指引；每次 Wiki Run 会冻结 skill 内容摘要。",
    skillKind: "类型",
    skillDigest: "摘要",
    skillPath: "路径",
    skillName: "名称",
    skillUnavailable: "无法获取 Skill 信息。",
    skillFork: "创建 / 刷新 fork",
    skillWorking: "处理中…",
    skillBundled: "使用默认（home/包）",
    skillLoadFile: "加载文件",
    skillSaveFile: "保存文件",
    skillFileLabel: "Skill 文件（fork 编辑器）",
    skillListRoot: "列出根目录",
    skillFiles: "文件：",
    rootPath: "根路径",
    selectedModelId: "所选模型 ID",
    modelProfile: "模型配置",
    dangerTitle: "危险操作",
    dangerDescription:
      "从应用索引中移除此工作区。可选删除 .okf-wiki 元数据目录（不会删除整个项目树）。",
    deleteWorkspace: "删除工作区",
    deleteConfirmTitle: "删除工作区？",
    deleteConfirm: "将工作区「{name}」从最近列表删除？此操作无法从界面撤销。",
    deleteMeta: "同时删除 .okf-wiki/",
    skillForked: "Skill fork 已就绪",
    skillSaved: "Skill 文件已保存",
  },
  sources: {
    breadcrumbWorkspaces: "工作区",
    breadcrumbSources: "源码",
    title: "源码",
    description:
      "关联已有本地 Git 检出（绝对路径，可在工作区根外），或将远程仓库克隆到本工作区的 sources/<id>。克隆使用本机 git 凭证；token 不会写入工作区配置。",
    loading: "加载源码…",
    registered: "已注册源码",
    probeAll: "全部探测",
    probing: "探测中…",
    empty: "尚无源码。请添加已有本地 Git 检出的绝对路径。",
    colId: "ID",
    colOrigin: "来源",
    colPath: "路径",
    colProbe: "探测",
    colIgnores: "忽略规则",
    originPath: "路径",
    originClone: "克隆",
    delete: "删除",
    removing: "移除中…",
    deleteConfirmTitle: "删除源码？",
    deleteConfirmBody:
      "从本工作区移除源码「{id}」？关联的本地路径不会从磁盘删除；工作区内的克隆目录可能仍会保留。",
    deleteSubmit: "删除源码",
    linkTitle: "关联已有路径",
    pathLabel: "路径（绝对）",
    pathPlaceholder: "D:/src/repo",
    sourceIdLabel: "源码 ID",
    sourceIdOptional: "（可选 slug）",
    sourceIdPlaceholder: "app",
    sourceIdHint: "小写 slug。留空则从路径推导。",
    addSource: "添加源码",
    adding: "添加中…",
    cloneTitle: "克隆到工作区",
    cloneHint: "克隆到 {root}/sources/<id>。",
    remoteUrl: "远程 URL",
    cloneId: "源码 ID",
    cloneRef: "引用（可选）",
    cloneSubmit: "克隆源码",
    cloning: "克隆中…",
    ignoreTitle: "忽略规则",
    ignoreDescription:
      "仓库相对的 POSIX glob。默认会排除依赖/构建/缓存等噪声（可关闭）。用户规则始终叠加。测试文件默认不排除。",
    applyDefaults: "应用默认源码忽略",
    ignorePatterns: "自定义忽略模式（每行一条）",
    ignorePlaceholder: "src/test/**\n**/*Test.java",
    presets: "预设",
    presetJava: "Java 测试",
    presetJs: "JS/TS 测试",
    presetPython: "Python 测试",
    saveIgnores: "保存忽略规则",
    savingIgnores: "保存中…",
    editIgnores: "编辑忽略",
    closeEditor: "关闭",
    defaultsOn: "默认开",
    defaultsOff: "默认关",
    customCount: "{n} 条自定义",
  },
  globalSettings: {
    title: "设置",
    description: "本机模型配置与凭证。工作区配置从不保存 API 密钥。",
    loading: "加载设置…",
    refresh: "刷新",
    addModel: "添加模型",
    tabModels: "模型",
    tabApp: "应用",
    tabDiagnostics: "诊断",
    skillsTitle: "Producer Skill（Agent Skills）",
    skillsDescription:
      "可移植 Agent Skills 布局（与 Codex/Grok 一致）：项目 `.agents/skills`，再用户 `~/.agents/skills`（下方开关，首次从产品包 seed）。关闭用户 skill 后，无项目 skill 时回退到包内资产。",
    loadHomeSkills: "从用户 ~/.agents/skills 加载 skill",
    loadHomeSkillsHint:
      "关闭后跳过 ~/.agents/skills，回退到包内资产（项目 .agents/skills 仍优先）。",
    homeSkillsPath: "用户 skill 路径",
    workspaceSkillsPath: "项目 skill 路径",
    skillsSaving: "保存中…",
    skillsSaved: "已保存",
    appSettingsUnavailable: "无法加载应用设置。",
    modelsTitle: "模型",
    modelsCount: "已配置 {n} 个",
    defaultSet: "已设默认",
    noDefault: "无默认",
    modelsEmpty: "还没有模型。请至少添加一个 OpenAI 兼容模型，创建工作区时再选择。",
    colName: "名称",
    colModelId: "模型 ID",
    colShape: "接口形态",
    colBaseUrl: "Base URL",
    colKey: "密钥",
    colMaxContext: "最大上下文",
    colActions: "操作",
    defaultBadge: "默认",
    setDefault: "设为默认",
    edit: "编辑",
    delete: "删除",
    envFallback: "环境回退：OPENAI_BASE_URL={base} · OPENAI_API_KEY={key}",
    envSet: "已设置",
    envUnset: "未设置",
    keySet: "已设置",
    editorCreateTitle: "添加模型",
    editorEditTitle: "编辑模型",
    displayName: "显示名称",
    displayNamePlaceholder: "企业 GPT-4o",
    modelIdLabel: "模型 ID",
    modelIdPlaceholder: "openai/my-served-model",
    modelIdHint:
      "OpenAI 兼容网关的模型标识（如 openai/gpt-4o 或裸部署名）。目前仅支持 OpenAI 兼容 provider。",
    baseUrl: "Base URL",
    baseUrlPlaceholder: "https://gateway.example.com/v1",
    apiKey: "API 密钥",
    apiKeyKeepPlaceholder: "留空则保留已存密钥",
    apiKeyPlaceholder: "sk-… 或网关令牌",
    clearApiKey: "清除已存 API 密钥",
    apiShape: "API 形态",
    shapeCompletions: "Chat Completions",
    shapeResponses: "Responses",
    maxContextTokens: "最大上下文（tokens）",
    maxContextTokensHint:
      "该模型的提供方硬上下文窗口。生成 Wiki 时占用约 85%（或工作区上下文目标）会自动压缩历史。",
    maxContextTokensPlaceholder: "例如 128000",
    saving: "保存中…",
    saveCreate: "添加模型",
    saveEdit: "保存更改",
    testing: "测试中…",
    testConnection: "测试连接",
    testOk: "可达",
    testFail: "失败",
    healthTitle: "API 连接",
    apiBase: "API 基址",
    apiBaseSameOrigin: "（同源）",
    health: "健康检查",
    healthNotChecked: "尚未检查",
    healthOk: "正常 · {service}",
    healthNotOk: "异常",
    checking: "检查中…",
    runHealthCheck: "运行健康检查",
    doctorTitle: "Doctor",
    status: "状态",
    statusOk: "正常",
    statusNotOk: "异常",
    node: "Node",
    platform: "平台",
    git: "Git",
    gitAvailable: "可用",
    gitUnavailable: "不可用",
    doctorModels: "模型",
    doctorModelsConfigured: "已配置 {n} 个",
    doctorNoCredentials: "无凭证",
    deleteConfirmTitle: "删除模型？",
    deleteConfirmBody:
      "删除模型「{name}」？已选择该模型的工作区会保留最后已知的模型 ID。",
    deleteSubmit: "删除模型",
    deleting: "删除中…",
    statusModelAdded: "Model added",
    statusModelUpdated: "模型已更新",
    statusModelDeleted: "模型已删除",
    statusDefault: "默认：{name}",
  },
  wikiLang: {
    en: "英文",
    zh: "中文",
  },
  session: {
    breadcrumbWorkspaces: "工作区",
    breadcrumb: "会话",
    title: "会话",
    description:
      "用于 Wiki 规划、工具与决策的对话式工作区。选项由智能体动态生成，不是固定产品按钮。",
    runPrefix: "运行",
    openRuns: "运行任务",
    loading: "加载会话…",
    stop: "停止",
    emptyTitle: "开始 Wiki 对话",
    emptyDescription: "试试「generate a wiki plan」——然后选择动态选项或输入修订说明。",
    choiceOnly: "请先选择上方选项以继续（此步骤禁用自由文本）。",
    planReviseHint: "在输入框写下对计划的修改意见，或选择上方的批准 / 拒绝。",
    placeholderNoSources: "请先添加源码（源码页）",
    placeholderChoice: "请选择上方选项…",
    placeholderPlanRevise: "描述计划修改（例如：增加 concepts.md）…",
    placeholderDefault: "发消息…（/ 命令，Tab 补全，↑↓ 选择）",
    sessions: "会话列表",
    switchSession: "切换会话",
    newSession: "新建会话",
    creatingSession: "创建中…",
    deleteSession: "删除",
    deletingSession: "删除中…",
    deleteConfirmTitle: "删除会话？",
    deleteConfirmBody: "删除此会话及其对话历史？此操作无法撤销。",
    deleteConfirmSubmit: "删除会话",
    slashHeading: "命令",
    slashEmpty: "无匹配命令",
    slashTooltip: "斜杠命令（Tab 补全）",
    lifecycle: {
      active: "进行中",
      waiting: "等待中",
      running: "运行中",
      completed: "已完成",
      failed: "失败",
    },
    chatStatus: {
      ready: "就绪",
      submitted: "发送中…",
      streaming: "生成中…",
      error: "错误",
    },
    phase: {
      idle: "空闲",
      planning: "规划中",
      awaiting_plan: "等待计划确认",
      writing: "写入中",
      awaiting_publish: "等待发布确认",
      done: "完成",
      failed: "失败",
    },
    tools: {
      subagent: "子代理",
      scope: "范围",
      subagentRunning: "子代理运行中…",
      noReceipt: "尚无回执",
      advancedRaw: "高级原始数据",
      truncatedPreview: "预览已截断",
      pagesWritten: "已写入 {written}/{total} 页",
      reviewClean: "审阅委员会：通过",
      reviewDefects: "审阅委员会：{n} 个问题，{blocking} 个阻塞",
      reviewRound: "第 {n} 轮",
      batchListedDirs: "已列出 {n} 个目录",
      batchReadSources: "已读取 {n} 个源文件",
      batchListedSkill: "Skill 列表 × {n}",
      batchReadSkill: "已读 {n} 个 skill 文件",
      batchListedWiki: "Wiki 列表 × {n}",
      batchReadWiki: "已读 {n} 个 wiki 页",
      batchGeneric: "{name} × {n}",
    },
  },
  runs: {
    breadcrumbWorkspaces: "工作区",
    breadcrumb: "任务",
    title: "任务",
    descriptionBefore:
      "Wiki Run 任务审计：查看状态、取消与重试。交互式生成、计划协商与发布决策在",
    descriptionLink: "会话",
    descriptionAfter: "页。",
    loading: "加载任务…",
    generateTitle: "生成 Wiki",
    cancel: "取消任务",
    cancelling: "取消中…",
    retry: "手动重试",
    retrying: "重试中…",
    startHeadless: "无界面启动",
    starting: "启动中…",
    startHeadlessTitle: "不经会话聊天的无界面任务（API / 审计路径）",
    generateInSession: "在会话中生成",
    advancedTitle: "高级",
    advancedHint: "不经会话聊天的无界面启动与任务审计控件。",
    needSource: "启动任务前请至少添加一个源码。",
    openSources: "打开源码",
    lastStatus: "最近任务状态",
    error: "错误",
    summary: "摘要",
    pages: "页面",
    runId: "任务 ID",
    created: "创建时间",
    jobEvents: "任务事件",
    jobEventsHintBefore:
      "状态与 wiki-run 工作流步骤（与会话相同）。详细工具/token 流在",
    jobEventsHintLink: "会话",
    jobEventsHintAfter: "页查看。",
    noEvents: "暂无事件。",
    skillDigest: "skill 摘要",
    publishReadyBefore: "暂存已就绪，可审阅。批准后发布到 ",
    publishReadyAfter: "；拒绝则保留暂存，不改动已发布 Wiki。",
    approvePublish: "批准发布",
    working: "处理中…",
    decline: "拒绝",
    gateOnSessionPlan:
      "此任务等待计划确认。请在会话中批准、修订或拒绝。",
    gateOnSessionPublish:
      "暂存已就绪。请在会话中发布或保留暂存。",
    openSessionToDecide: "打开会话进行决策",
    noRuns: "此工作区还没有任务记录。",
    recentTitle: "最近任务",
    recentEmpty: "你启动的任务会出现在这里。",
    colRunId: "任务 ID",
    colStatus: "状态",
    colError: "错误",
    colCreated: "创建时间",
  },
  wiki: {
    breadcrumbWorkspaces: "工作区",
    breadcrumb: "Wiki",
    title: "已发布 Wiki",
    description: "只读浏览工作区发布路径下的页面。",
    loading: "加载已发布 Wiki…",
    emptyTitle: "尚未发布",
    emptyDescription:
      "发布路径下未找到 Markdown 页面。请在会话中生成 Wiki 并批准发布以填充此视图。",
    goToSession: "在会话中生成",
    pages: "页面",
    pagesAria: "Wiki 页面",
    loadingPage: "加载页面…",
    selectPage: "从列表中选择页面。",
  },
  planConfirm: {
    title: "提议的页面计划",
    notes: "备注",
    pagesLabel: "页面",
    approve: "批准计划并写入",
    working: "处理中…",
    decline: "拒绝计划",
    revise: "提出修改意见",
    reviseHint: "描述希望如何修改页面计划。",
    revisePlaceholder: "例如：增加 concepts.md，去掉 architecture.md",
    reviseSubmit: "提交修改意见",
    fullscreen: "全屏查看计划",
    chipWrite: "写入 {n} 页",
    chipRevise: "提出修改意见",
    chipDeny: "拒绝此计划",
    chipPublish: "发布暂存 Wiki",
    chipKeepStaging: "仅保留暂存",
  },
  modelSelect: {
    label: "模型",
    emptyBefore: "尚未配置模型。请先",
    emptyLink: "在设置中添加模型",
    emptyAfter: "。",
    selectPlaceholder: "选择模型…",
    defaultSuffix: "（默认）",
    hintBefore: "模型在",
    hintLink: "设置",
    hintAfter: "中管理。Base URL 与 API Key 不按工作区设置。",
  },
  errorBanner: {
    title: "请求失败",
    dismiss: "关闭",
    unknown: "未知错误",
  },
  runStatus: {
    running: "运行中",
    published: "已发布",
    needs_input: "需要输入",
    failed: "失败",
    cancelled: "已取消",
    awaiting_plan: "等待计划确认",
    awaiting_publication: "等待发布确认",
    publication_declined: "已拒绝发布",
  },
  loading: {
    default: "加载中…",
  },
  agentWorkspace: {
    title: "智能体工作区",
    loading: "加载智能体工作区…",
    overview: "概览",
    workspaceSettings: "设置",
    sessions: "会话",
    newSession: "新建",
    creatingSession: "创建中…",
    noSessions: "暂无会话。创建一个以开始。",
    deleteSession: "删除会话",
    panels: "上下文面板",
    panelSources: "源码",
    panelWiki: "Wiki",
    panelPlan: "计划",
    panelRun: "运行",
    openFull: "打开",
    sourcesEmpty: "尚未关联源码。添加 Git 检出后即可生成 Wiki。",
    publicationPath: "发布路径",
    wikiLanguage: "Wiki 语言",
    wikiHint: "在完整 Wiki 视图中浏览已发布页面。",
    planEmpty: "尚无计划。启动 Wiki 运行以提出页面。",
    linkedRun: "关联运行",
    phase: "阶段",
    runsEmpty: "此工作区尚无任务。",
    emptyTitle: "智能体工作区",
    emptyDescription:
      "向智能体发消息或启动 Wiki 运行。工具调用与门控会流式显示在此时间线。",
    placeholder: "给智能体发消息…（Enter 发送，Shift+Enter 换行）",
    startWikiRun: "启动 Wiki 运行",
    wikiModel: "Wiki 模型",
    wikiModelHint: "本次 Wiki 运行使用的模型（来自 Settings 目录）。",
    send: "发送",
    stop: "停止",
    statusReady: "就绪",
    statusBusy: "处理中…",
    statusError: "错误",
    roleUser: "你",
    roleAssistant: "智能体",
    roleSystem: "系统",
    roleTool: "工具",
    toolInput: "输入",
    toolOutput: "输出",
    thinking: "思考",
    thinkingStreaming: "思考中…",
    jumpToLatest: "跳到最新",
  },
};
