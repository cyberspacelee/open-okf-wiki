...
def main() -> int:
    ...
    prompt = (
        ...
        "The multi-source Run has one Source scout per code Source ready. Stay "
        "coordinator-only: dispatch all ready scouts in parallel, then dispatch "
        "plan:workspace after every Source Brief completes. Preserve each task-start "
        "attempt token, consume path-only Handoffs, and repair every rejected State Gate. "
