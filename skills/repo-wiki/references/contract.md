...
Derive writes only under the run's proposals directory — never into a source
repository. One `agents-block-<source>.md` per Git source: exactly one
managed block, at most 15 non-empty inner lines. Include a Verify command
only if a worker ran it successfully during derive. CONTEXT terms and ADR
rationale always require human ratification.
