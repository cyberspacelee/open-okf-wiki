"""Run prepare: make a Wiki Run run-ready (mounts + frozen source/skill).
...
:class:`~okf_wiki.run.models.WikiRunRequest` is assembled (YAML / Manual Retry /
...
class PreparedMounts:
    """Run volumes resolved before snapshot/skill freeze (and before the publish lock)."""
...
def prepare_run(
    ...
    *,
    ...
    """Materialize snapshots, write inventory, freeze skill; emit run events.
