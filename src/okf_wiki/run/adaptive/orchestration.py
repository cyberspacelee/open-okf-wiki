...
class AdaptiveOrchestrator:
    ...
    def validate_completion(self) -> None:
        """run-facing completion gate (no framework types)."""
        if self.metrics.unresolved_failures():
...
@dataclass(slots=True)
class RootAssembly:
    """Deep run-boundary handle for one assembled Root + recursive delegation tree."""
    ...
    def staging_reviewer(self) -> WikiReviewer | None:
        """Run Boundary pre-publish Wiki Reviewer adapter, or None when disabled."""
    ...
    def topology_snapshot(self) -> dict[str, object]:
        """run-facing topology summary for tests (no Harness capability walk)."""
        return {
