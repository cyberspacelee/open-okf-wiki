...
from .policy import RUN_PUBLISH_REVIEWER_NODE_ID, NodeState, Role
...
class _AdaptiveMetrics:
    ...
    # run-side queue time waiting on concurrency gates (not model latency).
    ...
    def unresolved_failures(self) -> int:
        # Run Boundary pre-publish Reviewer soft-fails independently of research-tree health.
        return sum(
            state in {"partial", "failed", "cancelled"}
            for node_id, state in self.node_states.items()
            if node_id != RUN_PUBLISH_REVIEWER_NODE_ID
        )
...
@dataclass(slots=True)
class AdaptiveDeps:
    """Dependencies shared by run receipt tools and bounded child wrappers."""
