"""Run Boundary pre-publish Wiki Reviewer (publication gate), distinct from mid-run roster."""
...
from .policy import RUN_PUBLISH_REVIEWER_NODE_ID, AdaptivePolicy
...
class WikiReviewer:
    ...
    Holds Run Boundary pre-publish wiring so lifecycle / finalize never see AdaptiveDeps,
    ...
    async def review_staging(self, *, emit: Callable[..., None]) -> ReviewDefectsSummary:
        return await run_wiki_reviewer(
            model=self.model,
...
async def run_wiki_reviewer(
    *,
    ...
    """Run the Wiki Reviewer once before the publication approval gate.

    Soft-fails into a ``failed`` summary so the operator can still approve/deny;
    Run Boundary mechanical validation remains independent and must already have passed.
    """
    emit(
        "review_started",
        {"node_kind": "reviewer", "status": "running"},
        node_id=RUN_PUBLISH_REVIEWER_NODE_ID,
    )
    ...
    subagent = _make_reviewer(
        ...
        metrics=metrics,
        emit=emit,
        task_id=RUN_PUBLISH_REVIEWER_NODE_ID,
        # Run Boundary runs the Reviewer once; intentional single retry stays inside the agent.
        max_calls=2,
    )
    prompt = (
