"""Run Boundary-mediated receipt toolset and failure-receipt helpers."""
...
class _ReceiptToolset(FunctionToolset[AdaptiveDeps]):
    """Run Boundary-mediated receipt access; no Analysis Workspace mount is exposed."""
    ...
    def publish_receipt(
        ...
        *,
        ...
        if run_id != deps.run_id or node_id != deps.node_id or parent_id != deps.parent_id:
            raise ModelRetry("Receipt identity does not match the run assignment")
        if attempt != deps.attempt:
            raise ModelRetry(f"Use the run-assigned receipt attempt {deps.attempt}")
    ...
    def publish_fallback_receipt(
        ...
        *,
        ...
        if run_id != deps.run_id:
            raise ModelRetry("Fallback receipt run_id does not match the run assignment")
        if deps.metrics.node_parents.get(node_id) != deps.node_id:
            raise ModelRetry("Fallback target is not an assigned child of this run")
        if parent_id != deps.node_id:
            raise ModelRetry("Fallback parent_id does not match the run assignment")
        if task_id != node_id:
            raise ModelRetry("Fallback task_id and node_id must match the run assignment")
        if attempt != deps.metrics.next_attempts.get(node_id):
            expected = deps.metrics.next_attempts.get(node_id, 1)
            raise ModelRetry(f"Use the run-assigned fallback receipt attempt {expected}")
        if (artifact_name is None) != (artifact_markdown is None):
            raise ModelRetry("Provide both artifact_name and artifact_markdown, or neither")
        artifacts: dict[str, str] = {}
        descriptors: list[ReceiptArtifact] = []
...
def _require_deps(ctx: RunContext[AdaptiveDeps]) -> AdaptiveDeps:
    if not isinstance(ctx.deps, AdaptiveDeps):
        raise ModelRetry("run orchestration dependencies are unavailable; stop delegation")
    return ctx.deps
