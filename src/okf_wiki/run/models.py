...
1. **YAML** — :meth:`WikiRunRequest.from_yaml` → :mod:`okf_wiki.run.config`
2. **Manual Retry** — :meth:`WikiRunRequest.from_run_record` → :mod:`okf_wiki.run.records`
...
Run readiness after a request exists is owned by :mod:`okf_wiki.run.prepare`
(``prepare_mounts`` / ``prepare_run``), not by inventing a second validation layer.

:class:`WikiRunResourceLimitError` is defined in :mod:`okf_wiki.run.errors` and
re-exported here for import compatibility (``from okf_wiki.run.models import …``).
"""
...
# run-owned Default Source Ignores (noise). Tests are intentionally not listed.
...
class WikiRunRequest(BaseModel):
    ...
    * direct construction — tests / evaluation / CLI programmatic builds
    ...
    # Does not skip run validation, mounts, or publication locks. Off by default.
