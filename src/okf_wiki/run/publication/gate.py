"""run publication approval gate (Pydantic AI deferred-tool shapes).

After run validation of the Staging Wiki, publication is not a silent side
effect of structured ``Complete``. The Run Boundary models the publish decision as a
...
# Default operator-facing deny message when the Run Boundary builds ToolDenied results.
...
def build_publish_approval_request(
    *,
    ...
    """Build a run-owned deferred approval request for atomic publication.
...
def decision_from_results(
    ...
    """Map deferred tool results to a run publication decision."""
    if results is None:
        return "awaiting"
