"""Publication finalize: pre-publish review → publication gate → filesystem publish.
...
1. Optional Wiki Reviewer via :class:`StagingReviewer` (or precomputed defects)
...
run-owned gate using pydantic-ai deferred shapes; not agent-inline deferred
...
class StagingReviewer(Protocol):
    """Run Boundary pre-publish Wiki Reviewer seam (protocol in publication).

    Adaptive implements this with :class:`~okf_wiki.run.adaptive.reviewer.WikiReviewer`.
...
class PublicationOutcome:
    """Unified result of run publication finalize.
...
class PublicationContext:
    """Inputs for one run publication finalize decision.
...
async def finalize(context: PublicationContext) -> PublicationOutcome:
    """Run pre-publish review, publication approval gate, and optional filesystem publish.
