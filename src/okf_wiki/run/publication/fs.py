...
def published_repository_views(
    ...
    """Project Repository Snapshots into run publication provenance views."""
    return tuple(
...
def stage_published_wiki_for_refresh(
    ...
    """Load a run-owned real-directory Published Wiki into empty Staging for Refresh."""
    ...
    try:
        ...
        raise operator_error(
            "Refresh Published Wiki is not readable", error, error_cls=RunValidationError
        ) from error
    if not release.is_dir() or release.is_symlink():
        raise RunValidationError("Refresh requires an existing producer-managed Published Wiki")

    metadata_path = release / PUBLICATION_METADATA_NAME
    if metadata_path.is_symlink() or not metadata_path.is_file():
        raise RunValidationError("Refresh Published Wiki metadata is missing or not a regular file")
    try:
        metadata = _PublicationMetadata.model_validate_json(metadata_path.read_bytes())
    except Exception as error:
        raise operator_error(
            "Refresh Published Wiki metadata is invalid", error, error_cls=RunValidationError
        ) from error
    ...
    if len(page_hashes) > limits.wiki_entries_limit:
        raise RunValidationError("Refresh Published Wiki exceeds the configured entry count limit")
    ...
    while stack:
        ...
        for entry in os.scandir(directory):
            ...
            # Skip run-owned Wiki Visualization artifacts under the reserved viz/ directory.
    ...
    if actual_files != expected_files:
        raise RunValidationError("Refresh Published Wiki files do not match its page manifest")
...
def _copy_regular_file_no_follow(
    ...
    try:
        ...
        raise RunValidationError(f"{label} is not a readable regular file") from error
    ...
    try:
        ...
        if not stat.S_ISREG(opened.st_mode) or (opened.st_dev, opened.st_ino) != (
            ...
            raise RunValidationError(f"{label} is not a readable regular file")
        if opened.st_size > max_bytes:
            raise RunValidationError(f"{label} exceeds the configured byte limit")
...
def summarize_wiki_changes(
    ...
    """run-owned Staging vs prior Published Wiki change summary."""
...
def _publish_wiki(
    ...
    *,
    ...
    try:
        ...
        if errors:
            raise RunValidationError("Copied Wiki validation failed: " + "; ".join(errors))
