...
Import the finalize entrypoint from the submodule (or ``okf_wiki.run``) so the
``finalize`` module is not shadowed on this package::

    from okf_wiki.run.publication.finalize import finalize, PublicationOutcome
    from okf_wiki.run import finalize, PublicationOutcome
"""
