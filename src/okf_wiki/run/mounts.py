...
from .errors import RunValidationError, PublicationError, operator_error
...
# winnt.h reparse tags used for run path policy (portable; values are stable).
...
# These do not replace run roots the way junctions/symlinks do.
...
def _require_supported_runtime() -> None:
    """Portable capability check for run staging/publication (ADR 0017).
    ...
    if missing:
        raise RunValidationError(
            "okf-wiki Wiki Run requires portable filesystem primitives for atomic "
...
def _disallowed_path_reason(info: os.stat_result | object) -> str | None:
    """Return a short operator reason when a path component is run-disallowed.
    ...
    - On Windows reparse points, reject directory junctions and symlink reparse tags
      that redirect run roots; allow common cloud/filter tags (OneDrive, ProjFS, …)
...
def _path_component_error(label: str, reason: str) -> RunValidationError:
    return RunValidationError(f"{label} path must not contain {reason}")
...
def _legacy_symlink_publication_error(path: Path, *, for_refresh: bool = False) -> PublicationError:
    ...
    if for_refresh:
        return PublicationError(
            "Refresh requires an existing run-owned real-directory Published Wiki; "
...
def _prepare_mounts(request: WikiRunRequest) -> tuple[tuple[Path, ...], Path, Path, Path]:
    ...
    for index, source in enumerate(sources):
        if any(_overlaps(source, other) for other in sources[index + 1 :]):
            raise RunValidationError("Repository Snapshots must not overlap")
    ...
    if any(staging.iterdir()):
        raise RunValidationError("Staging Wiki must be empty")
    publication_input = request.publication.absolute()
    if publication_input.name in {"", ".", ".."}:
        raise RunValidationError("Published Wiki path must name a directory")
...
def _existing_directory(path: Path, label: str) -> Path:
    ...
    try:
        ...
        raise RunValidationError(f"{label} must be an existing directory") from error
    except OSError as error:
        raise operator_error(
            f"{label} path is not accessible", error, error_cls=RunValidationError
        ) from error
    if reason := _disallowed_path_reason(info):
        raise _path_component_error(label, reason)
    if not stat.S_ISDIR(info.st_mode):
        raise RunValidationError(f"{label} must be an existing directory")
...
def _check_directory_path(path: Path, label: str) -> None:
    if not path.is_absolute() or path.name in {"", ".", ".."} or ".." in path.parts:
        raise RunValidationError(f"{label} path must be a canonical directory path")
    ...
    for part in path.parts[1:]:
        ...
        if reason := _disallowed_path_reason(info):
            raise _path_component_error(label, reason)
        if not stat.S_ISDIR(info.st_mode):
            raise RunValidationError(f"{label} path must contain only directories")
...
def _create_directory_path(path: Path, label: str) -> None:
    ...
    for part in path.parts[1:]:
        ...
        try:
            ...
            try:
                ...
                raise operator_error(
                    f"{label} directory could not be created", error, error_cls=RunValidationError
                ) from error
            try:
                info = os.lstat(current)
            except OSError as error:
    ...
    if reason := _disallowed_path_reason(final):
        raise RunValidationError(f"{label} path changed during creation ({reason})")
    if not stat.S_ISDIR(final.st_mode):
        raise RunValidationError(f"{label} path changed during creation")
...
def _validate_release_root(path: Path) -> None:
    if os.path.lexists(path) and (_path_is_symlink_or_reparse(path) or not path.is_dir()):
        raise RunValidationError("Published Wiki release directory must be a regular directory")
...
def _ensure_same_volume_for_publication(publication: Path, releases: Path) -> None:
    ...
    try:
        ...
        raise operator_error(
            "Published Wiki parent is not accessible", error, error_cls=RunValidationError
        ) from error
    ...
    if os.path.lexists(publication) and publication.is_dir() and not publication.is_symlink():
        try:
            ...
            raise operator_error(
                "Published Wiki path is not accessible", error, error_cls=RunValidationError
            ) from error
...
def acquire_publication_lock(publication: Path) -> Path:
    """Exclusive run lock for one Wiki Run against a Published Wiki path (O_EXCL file).
