...
* **YAML path** (this module) — ``_wiki_run_request_from_yaml`` / ``WikiRunRequest.from_yaml``
  ...
  :mod:`okf_wiki.run.security`, and builds a :class:`~okf_wiki.run.models.WikiRunRequest`.
* **Manual Retry path** — :mod:`okf_wiki.run.records` (``_manual_retry_request`` /
  ``WikiRunRequest.from_run_record``) rebuilds a request from a terminal run record.
* **Programmatic construction** — construct :class:`~okf_wiki.run.models.WikiRunRequest`
  (and nested types/factories in :mod:`okf_wiki.run.models`) directly in tests, CLI
  helpers, or evaluation harnesses.

Starter YAML files are written by :mod:`okf_wiki.run.init_config`.

Run readiness after a request exists is **not** re-validated here. Use:

* :func:`okf_wiki.run.prepare.prepare_mounts` — path/skill/volume checks
* :func:`okf_wiki.run.prepare.prepare_run` — snapshot materialization + skill freeze
