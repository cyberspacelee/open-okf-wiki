"""Run-local, host-owned Analysis Receipt handoff storage."""
...
class AnalysisWorkspace:
    ...
    def register_node(self, task_id: str, node_id: str, parent_id: str | None = None) -> None:
        """Bind a Host-assigned task identity to the receipt node it may publish."""
        self._assert_open()
        ...
        *,
        ...
        with self._publish_lock:
            ...
            if receipt.attempt != self._next_attempts[task_id]:
                raise ValueError("receipt attempt does not match the Host assignment")
