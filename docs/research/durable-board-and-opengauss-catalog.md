# Durable Board and openGauss Catalog
...
3. **表结构按需取，不预载。** Anthropic 的 JIT retrieval 和 Amp 用 `psql`/`\\d` 现场看 schema 是同一条：名字可以先列，列/键/注释只在当前页面需要时再取。本仓库只支持 openGauss，连接串和 schema 写在 Workspace 上；表名模糊匹配，省略则允许该 schema 下全部表，但 prompt 里不倾倒 DDL。
