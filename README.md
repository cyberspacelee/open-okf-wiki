...
The installed command exposes four operations:

- `wiki-run` generates or refreshes a Wiki.
- `wiki-eval` runs deterministic or live producer evaluation.
...
before the report can recommend retaining the current single-Agent design or opening a capability
ticket. The evaluation command does not turn those signals into a claim of semantic proof.
