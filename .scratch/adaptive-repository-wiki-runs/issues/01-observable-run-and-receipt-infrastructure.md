...
- [x] Existing `WikiRunApplication.run(request) -> WikiRunResult`, JSON CLI output, validation, staging, and atomic publication behavior remain unchanged.
- [x] A run can emit ordered, bounded, content-free public events with run/node identifiers and secret redaction; events are diagnostics and never completion signals.
- [x] Every terminal run writes a bounded immutable Wiki Run Record containing exact non-secret frozen inputs, outcome, usage, retry counters, and publication status, without prompts, messages, receipts, source excerpts, or secrets.
- [x] Analysis Workspace lifecycle is run-local and cleaned after success, failure, or cancellation unless explicit diagnostic retention is requested.
