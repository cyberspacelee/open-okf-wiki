...
Keep `Written`, rejected hints, and evidence gaps current in the pre-created
draft while authoring so the receipt and remaining state survive compaction.

The draft is a compact Markdown receipt. Its H2 headings and the `complete`,
`blocked`, and `none` values below are machine schema tokens: copy them exactly
even when the Run language is not English. Write descriptive content in the Run
...
Validation returns one exhaustive, coded completion batch for
cited-file reads, Todo coverage, page placement, active contracts, and the
assigned target. Repair that batch in the durable draft and continue while the
issue set changes. This assignment-level result is not a global Candidate pass.
